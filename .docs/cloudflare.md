...
- Worker 变量 `TRIAL_DAILY_LIMITS="false"` 取消每身份、每 IP、全站三档每日试用额度的拒绝，计数仍累加（`daily_stats` 不受影响）；验证、速率限制、并发位、Live 时长和紧急开关仍生效。生产自 2026-09-17 起设为 `"false"`，见 [deployment.md](deployment.md)。
