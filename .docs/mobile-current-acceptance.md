...
- iPhone：本地签名 Release 39，已安装并启动；嵌入 JS，生产 API，测试标记与 OTA 关闭。
- Android：[EAS 19](https://expo.dev/accounts/jiahangzhang/projects/aside/builds/18115024-dfe0-4a7e-8671-4b38389d64ad)，固定签名密钥；覆盖安装与线上公开库检查通过。
- 后端：`f559178`，main 至 `25e1621`；Worker `bb4dd397-dcf6-49d1-8921-1c062ac179db`。
- [PR #29](https://github.com/qiz029/aside/pull/29) 仍为草稿。[代码 CI](https://github.com/qiz029/aside/actions/runs/35324139889)通过，包含 425 项应用测试、类型检查、Cloudflare 与媒体容器验证。
- 最近两项修复均在后端生效；既有手机安装无需更新。本文后的变更仅加强验收脚本与证据，没有修改产品界面或 Web 行为。
...
| 音频库、公开与私人音频、逐字稿、后台分析状态 | 当前 Android 19 线上公开库/逐字稿/Account 检查；双端私人上传与播放记录 | 未重复收费的真实长节目分析 |
...
| 文字发送清空、逐步显示回答、已有语音连接可朗读、纯文字不新建语音 | 双端唯一问题流式 UI 与最新消息截图；运行时和接口流式测试 | 外部模型输出为 fixture；清空和可见滚动由实际原生界面验证 |
...
| 中英文、深浅色、键盘、大字、小屏、错误与录音反馈 | 现有双端截图与原生流程；本轮重看小屏大字键盘、Android 对话、iOS 设置和当前恢复界面，未发现主操作被截断 | 审美最终由用户验收；底部控制区仍偏密集，不能仅凭自动化断言称为优质设计 |
| 测试、真实服务一次闭环、分发、分支与 PR | 当前代码 CI；实际用户外放成功与已保存的云端问答；iPhone 39 / Android 19、现有 PR 与生产发布记录 | 不把合成模型回归称为新增真实模型测试；PR 暂不标记 ready |
