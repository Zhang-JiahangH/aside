...
## Authorized production rollout — September 15, 2026

The user authorized publication to `https://asidefm.com`. The website and mobile-capable Worker are deployed as version `42ae2de6-2664-4c1a-a9e3-2e7c3bb52b92`. The native M4A switch is now persisted as `MOBILE_AUDIO_ENABLED=true` in production configuration. A fresh, private D1 backup was exported before the rollout; the mobile, question-usage and daily-stat migrations are applied, with no migrations pending. [Production rollout evidence](mobile-evidence/production-rollout.txt).

The media container rollout completed at 22:50 PDT with **100% of instances on target version 7** and no reported rollout errors. Its image digest is `sha256:e46e08855bdff04174aca980c4f2246f25951237c2b371e39857a058a11ed573`. The image is about 695 MB, reduced from 1.81 GB by excluding development dependencies, package caches and unnecessary graphics drivers. The actual release image passed boot, M4A-to-PCM conversion, invalid/overlong rejection, MP3 segmentation, JPEG cover extraction and cleanup; CI now runs that same image-level check. [Release image results](mobile-evidence/release-media-container.txt).

**Real signed-in end-to-end acceptance is still pending the mailbox verification code.** The installed online iOS Release app successfully requested a real email code through the production service. No fixed code or manually issued session is used. No paid model question has been made during this rollout. The remaining check is normal mailbox verification, one short real question, and website restoration of that account's completed conversation and progress. Publication and a successful email-send request do not establish that this complete flow passed.

The online Android APK and iOS simulator app remain the candidates described below. Their native code did not change in this server rollout. Apple device signing remains a separate installation prerequisite.

## Release readiness correction

Latest upstream synchronization: `b7f8694` adds durable daily usage snapshots before counter cleanup. Its import conflict with this branch is resolved, retaining mobile bearer authentication, recording decoding and versioned checkpoints. **34 Worker integration tests**, all TypeScript/boundary checks and the production Worker dry run passed. [Latest Worker results](mobile-evidence/stats-merged-cloudflare.txt). Native UI/runtime and website code are unchanged from the previous complete regression. `0007_daily_stats.sql` is now applied as part of the authorized rollout above.

**Initial production audit (historical):** On September 15 at 20:22 PDT, the production mobile email-start route returned `403 Origin required` without an Origin and `404` with the website Origin. Production D1 still listed `0006_mobile.sql` as unapplied. At that point the installed simulator binaries connected to temporary local data and fixed test codes; those fixtures could not demonstrate real email delivery or synchronization with website accounts. Subsequent online candidates and the authorized production rollout are recorded separately above.

Subsequent rollout preparation exported an owner-only, ignored production D1 backup and successfully applied `0006_mobile.sql`. Production health and website session endpoints still returned 200 afterward. An initial production deployment was rejected by automatic approval review. The user subsequently gave explicit production authorization, and the deployment has now completed as recorded above. Mailbox verification, production sign-in and website-account restoration still require the real acceptance flow; a fixture code must not be counted as that acceptance.
...
The user authorized detailed mock regression and requested one real end-to-end smoke to control model costs. Using the designated real mailbox, play an existing analyzed episode, ask one short voice question, and restore that account's progress and completed conversation on the website. Do not repeat paid questions automatically or analyze a long new upload for this check. Record actual delivery, answer playback and synchronized state; do not substitute a fixed code or canned model response. Real-service success is still pending.
...
Production Worker/container publication and the native recording switch are complete as recorded above. Apple device installation and distribution are still separate pending acceptance items.
