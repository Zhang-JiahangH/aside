...
GPT-Live requires incoming media (silence is sufficient) to advance text-driven speech. The RTC fixture now waits for an incoming frame before producing an answer; recvonly transport cannot pass voice acceptance. Run `python mobile/tests/test_rtc.py` in the fixture Python environment to check both the failing recvonly case and successful synthetic-silence case without paid API calls. The iOS native audio device supplies zero PCM without capturing the microphone. Android's recvonly voice transport remains outside this iOS fix and must not be considered accepted by a historical fixture pass.
