...
- `interaction.yaml`: start the fixture with `STREAM_DELAY_MS=6000`, use a fresh `EMAIL`, and verify that a partial text answer appears before the completed answer while the composer is already empty. This uses native HTTP streaming and the real Worker parser, with synthetic supplier chunks.
