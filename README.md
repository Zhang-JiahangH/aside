...
| `ASIDE_AUTO_RESUME_MS`     | `2000`          | 回答结束后的追问等待时间；`0` 禁用 |
...
Live 语音对话同样使用续播等待：后端通过 `answered` 事件确认答案已完整，回答音频播完后在等待时间内（默认 2 秒）没有人说话，节目自动继续。单靠静音不算回答结束，`answered` 到达之前的停顿（可能在思考或查询）不会触发续播，调试中的 `quiet` 也只表示当前无声。说“继续播放”或点击“继续收听”可以立即恢复，点“先别继续”则保持暂停。语音回答已经听完，不再套用长回答的延长等待。
